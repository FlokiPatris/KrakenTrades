...
def main() -> int:
    ...
    for dep in deps:
        ...
        for v in vulns:
            ...
            refs = v.get("references") or (adv.get("references") if isinstance(adv, dict) else []) or []
            severity = (v.get("severity") or (adv.get("severity") if isinstance(adv, dict) else "") or "").lower()
