...
from kraken_core import KrakenAPI, MarketData, custom_logger
...
def fetch_historical_prices(
    token_id: str, vs_currency: str = "eur", days: int = 30
) -> List[float]:
    try:
        resp = session.get(
...
# =========================
def fetch_coingecko_data_bulk(
    token_ids: List[str], vs_currency: str = "eur"
) -> Dict[str, MarketData]:
    ...
    try:
        ...
        for data in data_list:
            ...
            md.price_change_percentage_24h = float(
                data.get("price_change_percentage_24h", 0)
            )
            md.market_cap_change_percentage_24h = float(
                data.get("market_cap_change_percentage_24h", 0)
            )
            ...
            md.volatility_30d, md.momentum_30d = calculate_volatility_momentum(
                prices_30d
            )
...
def fetch_kraken_data(pair: str) -> dict:
    ...
    try:
        resp = session.get(
            KrakenAPI.URL, params={"pair": code}, timeout=KrakenAPI.TIMEOUT
        )
        resp.raise_for_status()
...
def fetch_market_data(
    pair: str, token_id: str, cg_map: Dict[str, MarketData]
) -> MarketData:
...
def fetch_bulk_market_data(
    pairs: List[str], token_map: Dict[str, str]
) -> Dict[str, MarketData]:
